...
export const HeroImmersive = () => {
  ...
  // Scale down the inner hero content as we scroll through the 170vh container
  const scale = useTransform(scrollYProgress, [0, 1], [1, 0.8]);
  const borderRadius = useTransform(scrollYProgress, [0, 1], ["0px", "40px"]);

  return (
    <section ref={containerRef} className="relative h-[170vh] w-full bg-ivory">
      {/* Sticky container that holds the scalable frame */}
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        <motion.div
          style={{ scale, borderRadius }}
          className="relative h-full w-full overflow-hidden bg-obsidian origin-top shadow-warm-lg"
        >
          {/* Background Image */}
          <motion.div
